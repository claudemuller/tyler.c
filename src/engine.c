...
void mouse_move(event_t ev_data)
{
    printf("the mouse moved at: %d\n", ev_data.x);
}

void load_tilemap(event_t ev_data)
{
    printf("load the tilemap()!\n");
}

void init(engine_t *engine, const bool debug)
{
    eventbus = init_eventbus();
    subscribe(&eventbus, EVENT_MOUSE_MOVE, &mouse_move);
    subscribe(&eventbus, EVENT_LOAD_TILEMAP, &load_tilemap);
